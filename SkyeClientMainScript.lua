...
local Window = Library.CreateLib("Skye Client | Public | BETA", "GrapeTheme")
local Tab = Window:NewTab("Main")
local Tab2 = Window:NewTab("Credits")
local Section = Tab:NewSection("Combat")
local Section2 = Tab2:NewSection("Credits")
Section2:NewLabel("Credits")
Section2:NewLabel("Code: 7GrandDadPGN with Vape V4")
Section:NewLabel("Combat")
Section:NewToggle("Reach", "Extends sword reach", function(state)
    if state then
        bedwars.CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE = ReachValue.Value + 3.3
			else
				bedwars.CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE = 14.4
			end
end)
Section:NewToggle("AutoSprint", "Don't hold that annoying rightshift key to sprint.", function(callback)
    if callback then
...
Section:NewToggle("Velocity", "Reduces knockback taken.", function(callback)
    if callback then
	...
	local function autoLeaveAdded(plr)
		task.spawn(function()
			...
			if getRole(plr) >= 100 then
                ...
                Section:NewToggle("AutoLeave", "Queues on it's own", function(callback)
    if callback then
...
Section:NewToggle("GrapplingHookDisabler", "ToggleInfo", function(callback)
    if callback then
...
Section:NewToggle("LongJump", "basically fly but faster", function(callback)
    if callback then
